...
export default function ForgotPasswordPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await unauthorizedFetch("/forgot-password", {
                method: "POST",
                body: JSON.stringify({ email })
            });

            if (res.ok) {
                showNotification("Pokud email existuje, odkaz byl odeslán.", "success");
                setEmail('');
            } else {
                const data = await res.json().catch(() => ({}));
                showNotification(data.message || "Došlo k chybě", "error");
            }
        } catch (error) {
    ...
    return (
        <div className="flex h-[100dvh] w-full flex-col justify-center px-6 lg:px-8 bg-visualy-dark overflow-hidden">
            <div className="sm:mx-auto sm:w-full sm:max-w-sm">
                ...
                />
                <h2 className="mt-10 text-center text-2xl/9 font-bold tracking-tight text-white">Zapomenuté heslo</h2>
            </div>
